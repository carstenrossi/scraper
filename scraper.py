...
def fetch_with_browser(url: str, timeout: int = 30000) -> str:
    """Fetch mit echtem Browser für JS-gerenderte Seiten."""
    browser = get_browser()
    page = browser.new_page()
    try:
        page.goto(url, timeout=timeout, wait_until="networkidle")
        # Warte kurz, damit JS fertig rendern kann
...
def extract_internal_links(soup: BeautifulSoup, base_url: str) -> list[dict]:
    """Extrahiert alle internen Links (gleiche Domain)."""
...
def scrape_page(url: str, referer: str = None, force_browser: bool = False, verbose: bool = False) -> tuple[str, str, list[dict], bool]:
    """
...
def crawl(start_url: str, max_pages: int = 50, max_depth: int = None, force_browser: bool = False, verbose: bool = False) -> tuple[list[dict], list[dict]]:
    """
    ...
    while to_visit and len(pages) < max_pages:
        ...
        title, content, links, success = scrape_page(url, referer=referer, force_browser=force_browser, verbose=verbose)
  ...
  python3 scraper.py https://example.com -c -m 100 -b -v    # Kompakt mit allen Optionen
    ...
    try:
        if args.crawl:
            ...
            print(f"🕷️  Starte Crawling von {args.url} (max {args.max} Seiten{depth_info})...\n")
